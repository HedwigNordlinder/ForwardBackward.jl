...
function endpoint_conditioned_sample(X0::SwitchState, X1::SwitchState, process::SwitchBridgeProcess, t::AbstractArray)
    ...
    @inbounds for ind in CartesianIndices(Xt.main_state.state)
        xt = endpoint_conditioned_sample(SwitchState(X0.main_state.state[ind], X0.switching_state.state[ind]), SwitchState(X1.main_state.state[ind], X1.switching_state.state[ind]), process, t[ind])
        Xt[ind] = xt
    end
    return Xt
end